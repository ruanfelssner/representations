            ...
            <div class="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-5 text-white flex-shrink-0">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <h3 class="text-xl font-bold">Registrar Nova Visita</h3>
                    <p class="text-sm text-emerald-100">{{ clienteNome }}</p>
                  </div>
                </div>
                <button
                  @click="$emit('close')"
              ...
              <form @submit.prevent="handleSubmit" class="space-y-5">
                ...
                <div class="bg-gray-50 rounded-xl p-4 border-2 border-dashed border-gray-300">
                  <label class="flex items-center gap-3 cursor-pointer">
                    <input
                      v-model="form.vendeuAlgo"
                      type="checkbox"
                      class="w-5 h-5 text-emerald-600 rounded focus:ring-emerald-500"
                    />
                    <span class="text-sm font-semibold text-gray-700">
                      💰 Realizou venda nesta visita
                    </span>
                  </label>
                </div>

                <!-- Campos de Venda (aparecem se vendeuAlgo = true) -->
                <Transition name="slide-down">
                  <div v-if="form.vendeuAlgo" class="space-y-4 bg-emerald-50 rounded-xl p-4 border border-emerald-200">
                    <!-- Valor da Venda -->
                    <div>
...
watch(() => form.value.vendeuAlgo, (vendeu) => {
  if (!vendeu) {
...
function adicionarProduto() {
  ...
  const produto = produtos.find(p => p.id === novoProduto.value.produtoId)
  if (!produto) return
